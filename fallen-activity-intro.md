...
活动开始前，服务器会同步玩家已确认的网页阵营选择。阵营统一结算后，仍未分配阵营的玩家首次进入时，会由 QAPI 自动加入当前人数最少的阵营；自动分配结果会持久保存，之后不会因重连或同步而换队。
...
A 阵营拥有两个固定传送站，用于连接旧主城与芙岛。传送站区域为 `6×3×6`，边界使用蓝色粒子描边。
...
活动进行期间，每日 01:00–07:00（北京时间）实施宵禁。宵禁开始时在线玩家会被断开，期间所有玩家均无法进入服务器，07:00 起恢复开放。
